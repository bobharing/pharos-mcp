...
export function registerAnalysisTools(server: McpServer) {
  ...
  server.registerTool(
    ...
    {
      description:
        "Page resource breakdown (images, JS, CSS, fonts) by type and size. Reads from cache (no new Lighthouse run) if pharos_audit already ran for this URL.",
  ...
  server.registerTool(
    ...
    {
      description:
        "Third-party breakdown by entity (analytics, ads, social, etc.) with byte and blocking-time impact. Reads from cache (no new Lighthouse run) if pharos_audit already ran.",
